    ...
    ILogger<SocialPacketHandler> logger) : IClientPacketHandler
{
    ...
    private async Task SayAsync(ClientConnection connection, byte[] payload, CancellationToken ct)
    {
        ...
        var packet = BuildChatBroadcast(sender.CharacterId, sender.Name, text);
        foreach (var player in world.GetPlayersInMap(sender.MapId))
            await connections.SendAsync(player.CharacterId, packet, ct);

        logger.LogDebug("Map chat CharacterId={CharacterId} MapId={MapId} Length={Length}", characterId, sender.MapId, text.Length);
    }
    ...
    private async Task InviteAsync(ClientConnection connection, byte[] payload, CancellationToken ct)
    {
        ...
        if (!world.TryGetPlayer(inviterId, out var inviter) || inviter is null ||
            !world.TryGetPlayer(targetId, out var target) || target is null ||
            inviter.MapId != target.MapId)
        {
    ...
    private async Task LeaveAsync(ClientConnection connection, CancellationToken ct)
    {
        ...
        if (!parties.Leave(characterId, out var remaining, out var affected))
        {
            await SendLeaveResultAsync(connection, false, "Not in a party.", ct);
            return;
        }

        await SendLeaveResultAsync(connection, true, "Left party.", ct);
    ...
    private static byte[] BuildPartyCleared()
        => PacketCodec.Encode(Opcode.PartyStateBroadcast, new byte[25]); // Guid.Empty + LeaderId=0 + Count=0

    private static byte[] BuildChatBroadcast(long characterId, string name, string text)
    {
        using var ms = new MemoryStream();
        using var writer = new BinaryWriter(ms, Encoding.UTF8, true);
        ...
        return PacketCodec.Encode(Opcode.ChatSayBroadcast, ms.ToArray());
    }
    ...
    private static Task SendLeaveResultAsync(ClientConnection connection, bool success, string message, CancellationToken ct)
    {
        ...
        return connection.SendAsync(Opcode.PartyLeaveResponse, payload, ct);
    }
