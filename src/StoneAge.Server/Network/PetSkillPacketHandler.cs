    ...
    ILogger<PetSkillPacketHandler> logger) : IClientPacketHandler
{
    ...
    public Task HandleAsync(ClientConnection connection, PacketFrame packet, CancellationToken ct)
    {
        ...
        {
            Opcode.PetSkillListRequest => SendListAsync(characterId, packet.Payload, connection, ct),
            Opcode.PetSkillLearnRequest => LearnAsync(characterId, packet.Payload, connection, ct),
            Opcode.PetSkillForgetRequest => ForgetAsync(characterId, packet.Payload, connection, ct),
            _ => Task.CompletedTask
        };
    }

    private async Task SendListAsync(long characterId, byte[] payload, ClientConnection connection, CancellationToken ct)
    {
        if (!TryReadPetId(payload, out var petId))
        {
            await SendResultAsync(connection, Opcode.PetSkillListResponse, false, "Invalid pet.", ct);
            return;
        }
        ...
        if (!ownsPet)
        {
            await SendResultAsync(connection, Opcode.PetSkillListResponse, false, "Pet not found.", ct);
            return;
        }
        ...
        await connection.SendAsync(Opcode.PetSkillListResponse, ms.ToArray(), ct);
    }

    private async Task LearnAsync(long characterId, byte[] payload, ClientConnection connection, CancellationToken ct)
    {
        if (payload.Length != 13)
        {
            await SendResultAsync(connection, Opcode.PetSkillLearnResponse, false, "Invalid request.", ct);
            return;
        }
        ...
        {
            await SendResultAsync(connection, Opcode.PetSkillLearnResponse, false, "Invalid skill or slot.", ct);
            return;
        }
        ...
        if (!ownsPet)
        {
            await SendResultAsync(connection, Opcode.PetSkillLearnResponse, false, "Pet not found.", ct);
            return;
        }

        if (await db.CharacterPetSkills.AnyAsync(x => x.CharacterPetId == petId && x.Slot == slot, ct))
        {
            await SendResultAsync(connection, Opcode.PetSkillLearnResponse, false, "Skill slot is occupied.", ct);
            return;
        }

        if (await db.CharacterPetSkills.AnyAsync(x => x.CharacterPetId == petId && x.SkillId == skillId, ct))
        {
            await SendResultAsync(connection, Opcode.PetSkillLearnResponse, false, "Pet already knows this skill.", ct);
            return;
        }
        ...
        await SendResultAsync(connection, Opcode.PetSkillLearnResponse, true, "Skill learned.", ct);
    }

    private async Task ForgetAsync(long characterId, byte[] payload, ClientConnection connection, CancellationToken ct)
    {
        if (payload.Length != 9)
        {
            await SendResultAsync(connection, Opcode.PetSkillForgetResponse, false, "Invalid request.", ct);
            return;
        }
        ...
        {
            await SendResultAsync(connection, Opcode.PetSkillForgetResponse, false, "Invalid pet or slot.", ct);
            return;
        }
        ...
        if (!ownsPet)
        {
            await SendResultAsync(connection, Opcode.PetSkillForgetResponse, false, "Pet not found.", ct);
            return;
        }
        ...
        if (row is null)
        {
            await SendResultAsync(connection, Opcode.PetSkillForgetResponse, false, "Skill slot is empty.", ct);
            return;
        }
        ...
        await SendResultAsync(connection, Opcode.PetSkillForgetResponse, true, "Skill forgotten.", ct);
    }
